...
var PrintNodeClient = function (options) {
  ...
  this.childAccountById = options.childAccountById || null;
  this.childAccountByEmail = options.childAccountByEmail || null;
  this.childAccountByCreatorRef = options.childAccountByCreatorRef || null;
};
...
PrintNodeClient.prototype._putJSON = function (path, body) {
  return this._makeRequest(path, {
    method: "PUT",
    body: body
  })
...
PrintNodeClient.prototype._deleteJSON = function (path) {
  return this._makeRequest(path, {
    method: "DELETE"
  })
}
...
PrintNodeClient.prototype._makeRequest = function (path, options) {
  var defaults = {
    ...
    json: true,
    ...(this.childAccountByEmail || this.childAccountById || this.childAccountByCreatorRef) && {
      headers: {
        ...(this.childAccountByEmail) && {'X-Child-Account-By-Email': this.childAccountByEmail},
        ...(this.childAccountById) && {'X-Child-Account-By-Id': this.childAccountById},
        ...(this.childAccountByCreatorRef) && {'X-Child-Account-By-CreatorRef': this.childAccountByCreatorRef}
      }
    }
  };
